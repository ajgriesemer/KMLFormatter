...
(async () => {
    ...
    for (let i = 0; i < Object.keys(zip.files).length; i++) {
        ...
        if(Object.keys(zip.files)[i].match(/\.[0-9a-z]+$/i)[0] == '.kml'){
            ...
            const placemark2DPolylineNode = xmlDOM.createElement('Placemark')
            const multiGeometry2DPolylineNode = xmlDOM.createElement('MultiGeometry')
            ...
            polylines.forEach((pl)=> {
                ...
                const coordinatesNode = multiGeometry2DPolylineNode.appendChild(xmlDOM.createElement('Polygon'))
